...
let feature_product_container = document.querySelector(
  ".feature-product-container"
);

let generate_Feature_Item = (feature_Get_All_Product) => {
  no_Have_Product.classList.remove("active");
  let all_Feature_product = feature_Get_All_Product
    .map((product) => {
      let { id, item } = product;
  ...
  let decrease_button_feature = document.querySelectorAll(
    ".feature-quantity .decrease"
  );
  decrease_button_feature.forEach((decrease_btn) => {
    decrease_btn.addEventListener("click", () => {
      let checkID = feature_Get_All_Product.find((item) => {
        return item.id == decrease_btn.id;
      });
      if (checkID.item <= 1) {
        console.log("er");
      } else {
        ...
        localStorage.setItem(
          "all_Product",
          JSON.stringify(feature_Get_All_Product)
        );
        generate_Feature_Item(feature_Get_All_Product);
  ...
  let increase_button_feature = document.querySelectorAll(
    ".feature-quantity .increase"
  );
  increase_button_feature.forEach((increase_btn) => {
    increase_btn.addEventListener("click", () => {
      let checkID = feature_Get_All_Product.find((item) => {
        return item.id == increase_btn.id;
      });
      checkID.item = checkID.item + 1;
      localStorage.setItem(
        "all_Product",
  ...
  delete_Product.forEach((delete_btn) => {
    delete_btn.addEventListener("click", () => {
      ...
      let checkId =
        feature_Get_All_Product.filter((product) => {
...
function calculateTotal(feature_Get_All_Product) {
  ...
  feature_Get_All_Product.map((item) => {
    let get_Data_Product = data.find((product) => {
