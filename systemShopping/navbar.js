...
window.amountProduct = function updateNavbar(get_All_Product_Storage) {
  ...
  let resultProduct = get_All_Product_Storage
    .map((product_item) => {
      return product_item.item;
    })
    .reduce((x, y) => {
      return x + y;
    }, 0);
...
===============================================================*/
let shoppingCartNavbar = document.querySelector(
...
window.updateLNavProduct = generateCartItems = (get_All_Product_Storage) => {
  ...
  let product_from_storage = product_Storage
    .map((product) => {
      let { item, id } = product;
      let checkData = data.find((checkProduct) => {
        return checkProduct.id == id;
      });
...
function deleteProductNavbar(getIDProduct, product_Storage) {
  ...
  let checkId =
    product_Storage.filter((product) => {
      return product.id !== getIDProduct;
    }) || [];
